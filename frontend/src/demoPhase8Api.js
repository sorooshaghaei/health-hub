...
function findSetup(store, code, { includeUsed = false } = {}) { return Object.values(store.setup_codes).find((item) => item.code === clean(code) && (includeUsed || !item.used_at) && Date.now() <= new Date(item.expires_at).getTime()) ?? null; }
