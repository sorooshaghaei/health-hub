...
function issueChallenge(store, account, { purpose, channel, pendingValue = null }) {
  ...
  const current = store.challenges[challengeKey(account.id, purpose)];
  if (current?.created_at && Date.now() - new Date(current.created_at).getTime() < RESEND_MIN_MS) fail({ detail: "Wait before requesting another verification code." });
...
async function verificationContactUpdate(store, staffToken, data) {
  const { session, account } = sessionFor(store, staffToken);
  if (account.email_verified_at && account.phone_verified_at) fail({ detail: "Use Account settings to change a verified account contact." }, 403);
  if (!["email", "phone"].includes(data.kind)) fail({ kind: ["Choose email or phone."] });
  if (await hashSecret(data.current_password ?? "") !== account.password_hash) fail({ detail: "Current password is incorrect." }, 403);
  session.reauthenticated_at = nowIso();

  if (data.kind === "email") {
    const value = normalizeEmail(data.value);
    if (!value.includes("@")) fail({ value: ["Enter a valid email address."] });
    if (value === account.email) fail({ value: ["Enter a different email address."] });
    if (store.accounts.some((candidate) => candidate.id !== account.id && candidate.email === value)) fail({ value: ["This email is already in use."] });
    account.email = value;
    account.email_verified_at = null;
    delete store.challenges[challengeKey(account.id, "email_verify")];
    delete store.challenges[challengeKey(account.id, "email_change")];
  } else {
    const value = normalizePhone(data.value);
    if (value === account.phone) fail({ value: ["Enter a different phone number."] });
    if (store.accounts.some((candidate) => candidate.id !== account.id && candidate.phone === value)) fail({ value: ["This phone number is already in use."] });
    account.phone = value;
    account.phone_verified_at = null;
    delete store.challenges[challengeKey(account.id, "phone_verify")];
    delete store.challenges[challengeKey(account.id, "phone_change")];
  }
  saveAuthStore(store);
  return { user: publicUser(store, account, session) };
}
...
function deviceAuthorizationRequest(store, staffToken, data) {
  ...
  const challenge = issueChallenge(store, account, { purpose: "device_authorize", channel: data.channel }); saveAuthStore(store); return challengeResponse(challenge);
}
...
async function contactChangeRequest(store, staffToken, kind, data) {
  ...
  const challenge = issueChallenge(store, account, { purpose: kind === "email" ? "email_change" : "phone_change", channel: kind === "email" ? "email" : "sms", pendingValue: value }); saveAuthStore(store); return challengeResponse(challenge);
}
...
function setupState(setup) { return setup ? { created_at: setup.created_at ?? null, expires_at: setup.expires_at } : null; }
function activeAssistantSetup(store, clinicId) { return Object.values(store.setup_codes).filter((item) => item.clinic_id === clinicId && !item.used_at && Date.now() <= new Date(item.expires_at).getTime()).sort((first, second) => new Date(second.created_at).getTime() - new Date(first.created_at).getTime())[0] ?? null; }
function assistantSetupStatus(store, staffToken) { const { session, account } = sessionFor(store, staffToken); const clinic = store.clinics.find((c) => c.id === session.clinic_id); if (!clinic || account.role !== "doctor" || clinic.owner_doctor_id !== account.id) fail({ detail: "Only the clinic Doctor can manage the Assistant slot." }, 403); return { active_setup: setupState(activeAssistantSetup(store, clinic.id)) }; }
function assistantSetup(store, staffToken, data) { const { session, account } = sessionFor(store, staffToken); const clinic = store.clinics.find((c) => c.id === session.clinic_id); if (!clinic || account.role !== "doctor" || clinic.owner_doctor_id !== account.id) fail({ detail: "Only the clinic Doctor can manage the Assistant slot." }, 403); const existing = currentAssistantMembership(store, clinic.id); if (existing && !data.replace_existing) fail({ detail: "This clinic already has an Assistant. Choose replacement explicitly." }, 409); if (existing) { existing.is_active = false; for (const [token, s] of Object.entries(store.sessions)) if (s.user_id === existing.user_id && s.clinic_id === clinic.id) delete store.sessions[token]; markDormantIfNeeded(store, existing.user_id); } for (const [id, setup] of Object.entries(store.setup_codes)) if (setup.clinic_id === clinic.id && !setup.used_at) delete store.setup_codes[id]; const id = crypto.randomUUID(), code = setupCode(); store.setup_codes[id] = { id, clinic_id: clinic.id, created_by_id: account.id, code, created_at: nowIso(), used_at: null, expires_at: expiresIn(SETUP_TTL_MS) }; saveAuthStore(store); return { setup_code: code, expires_at: store.setup_codes[id].expires_at, active_setup: setupState(store.setup_codes[id]) }; }
function findSetup(store, code, { includeUsed = false } = {}) { const normalizedCode = clean(code).toUpperCase(); return Object.values(store.setup_codes).find((item) => item.code === normalizedCode && (includeUsed || !item.used_at) && Date.now() <= new Date(item.expires_at).getTime()) ?? null; }
...
function assistantSetupClaim(store, staffToken, data) {
  ...
  const setup = findSetup(store, data.code, { includeUsed: true }); if (!setup) fail({ detail: "Assistant setup code is invalid or expired." }); const current = currentAssistantMembership(store, setup.clinic_id); if (current && current.user_id !== account.id) fail({ detail: "The Assistant slot is already filled." }, 409); if (setup.used_at && !current) fail({ detail: "Assistant setup code is invalid or expired." });
  ...
  let membership = current || membershipFor(store, account.id, setup.clinic_id, true); if (membership) membership.is_active = true; else { membership = { id: crypto.randomUUID(), user_id: account.id, clinic_id: setup.clinic_id, is_active: true, joined_at: nowIso() }; store.memberships.push(membership); }
...
export async function demoPhase8ApiRequest(path, { method = "GET", data = {}, staffToken, deviceToken } = {}) {
  ...
  if (pathname === "/api/clinic/assistant/setup/" && method === "GET") return assistantSetupStatus(store, staffToken);
