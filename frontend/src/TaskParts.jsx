...
export function TaskForm({ task, patients, saving, onSave, onCancel }) {
  ...
  return <form className="task-form" onSubmit={submit}>
    ...
    <label className="task-field"><span>Title</span><input name="title" value={form.title} onChange={update} maxLength={200} required /></label>
    ...
    <div className="form-actions"><button className="secondary-button" type="button" onClick={onCancel}>Cancel</button><button className="primary-button" disabled={saving}>{saving ? "Saving…" : task ? "Save changes" : "Create task"}</button></div>
  </form>;
}
