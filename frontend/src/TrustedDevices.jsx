...
function DeviceModal({ staffToken, onClose }) {
  ...
  return <Dialog onClose={onClose} returnFocusSelector="#workspace-account-trigger" ariaLabelledBy="trusted-devices-title" ariaDescribedBy="trusted-devices-description">
...
export default function TrustedDevices({ staffToken, onOpen }) {
  ...
  return <><Button id="trusted-devices-trigger" role="menuitem" type="button" disabled={open} onClick={() => { setOpen(true); onOpen?.(); }}>Devices</Button>{open && <DeviceModal staffToken={staffToken} onClose={() => setOpen(false)} />}</>;
}
