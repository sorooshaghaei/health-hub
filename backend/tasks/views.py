...
from accounts.permissions import active_membership_role
...
def active_task_for_request(request, task_id, *, lock=False):
    ...
    queryset = queryset.select_for_update() if lock else queryset.select_related("patient", "created_by", "completed_by")
    try: return queryset.get(pk=task_id)
    except SharedTask.DoesNotExist: raise NotFound("Task not found.")


def deleted_task_for_request(request, task_id, *, lock=False):
    clinic = clinic_for_staff(request)
    queryset = SharedTask.all_objects.filter(clinic=clinic, deleted_at__isnull=False)
    queryset = queryset.select_for_update() if lock else queryset.select_related("patient", "created_by", "completed_by")
    try: return queryset.get(pk=task_id)
    except SharedTask.DoesNotExist: raise NotFound("Deleted task not found.")


def active_comment_for_request(request, comment_id, *, lock=False):
    clinic = clinic_for_staff(request)
    queryset = TaskComment.objects.select_related("author", "task", "task__clinic").filter(task__clinic=clinic, task__deleted_at__isnull=True)
    if lock: queryset = queryset.select_for_update()
    try: return queryset.get(pk=comment_id)
    except TaskComment.DoesNotExist: raise NotFound("Comment not found.")


def deleted_comment_for_request(request, comment_id, *, lock=False):
    clinic = clinic_for_staff(request)
    queryset = TaskComment.all_objects.select_related("author", "task", "task__clinic").filter(task__clinic=clinic, task__deleted_at__isnull=True, deleted_at__isnull=False)
    if lock: queryset = queryset.select_for_update()
    try: return queryset.get(pk=comment_id)
    except TaskComment.DoesNotExist: raise NotFound("Deleted comment not found.")


def patient_for_task(clinic, patient_id):
    if patient_id is None: return None
    try: return Patient.objects.get(pk=patient_id, clinic=clinic)
...
class SharedTaskAttentionView(APIView):
    def get(self, request): return Response({"attention_required": task_attention_required(request)})
    def post(self, request):
        membership = request.auth.membership
        membership.task_attention_seen_at = timezone.now(); membership.save(update_fields=["task_attention_seen_at"])
        return Response({"attention_required": False, "seen_at": membership.task_attention_seen_at})
...
class SharedTaskDetailView(APIView):
    def get(self, request, task_id): return Response(SharedTaskSerializer(active_task_for_request(request, task_id)).data)
    def patch(self, request, task_id):
        task=active_task_for_request(request, task_id); ensure_task_creator(request, task); serializer=TaskWriteSerializer(data=request.data, partial=True); serializer.is_valid(raise_exception=True); data=serializer.validated_data
...
class SharedTaskDoneView(APIView):
    def post(self, request, task_id):
        with transaction.atomic():
            ...
            if task.status != SharedTask.Status.OPEN: return Response({"code":"task_already_done","detail":"This task is already done."}, status=status.HTTP_409_CONFLICT)
            task.mark_done(request.user)
        return Response({"code":"task_done","detail":"Task marked done.","task":SharedTaskSerializer(task).data,"undo_until":task.done_undo_until})
...
class SharedTaskUndoDoneView(APIView):
    def post(self, request, task_id):
        with transaction.atomic():
            task=active_task_for_request(request, task_id, lock=True)
            if task.status != SharedTask.Status.DONE or task.done_undo_until is None: return Response({"code":"task_not_done","detail":"This task is not done."}, status=status.HTTP_400_BAD_REQUEST)
            if timezone.now() > task.done_undo_until: return Response({"code":"undo_expired","detail":"The five-second Undo period has expired."}, status=status.HTTP_400_BAD_REQUEST)
            task.undo_done()
...
class TaskCommentCreateView(APIView):
    def post(self, request, task_id):
        task=active_task_for_request(request, task_id); serializer=TaskCommentWriteSerializer(data=request.data); serializer.is_valid(raise_exception=True); comment=TaskComment.objects.create(task=task, author=request.user, body=serializer.validated_data["body"])
...
class TaskCommentDetailView(APIView):
    def patch(self, request, comment_id):
        ...
        if comment.author_id != request.user.id: raise PermissionDenied("You can edit only your own comments.")
        serializer=TaskCommentWriteSerializer(data=request.data); serializer.is_valid(raise_exception=True); comment.body=serializer.validated_data["body"]; comment.edited_at=timezone.now(); comment.save(update_fields=["body","edited_at","updated_at"]); return Response(TaskCommentSerializer(comment).data)
    def delete(self, request, comment_id):
        with transaction.atomic():
            comment=active_comment_for_request(request, comment_id, lock=True)
            if comment.author_id != request.user.id: raise PermissionDenied("You can delete only your own comments.")
            comment.soft_delete()
        return Response({"code":"task_comment_deleted","detail":"Comment deleted.","comment_id":str(comment.id),"undo_until":comment.delete_undo_until})
...
class TaskCommentUndoDeleteView(APIView):
    def post(self, request, comment_id):
        with transaction.atomic():
            ...
            if comment.author_id != request.user.id: raise PermissionDenied("You can undo only your own comment deletion.")
            if timezone.now() > comment.delete_undo_until: return Response({"code":"undo_expired","detail":"The five-second Undo period has expired."}, status=status.HTTP_400_BAD_REQUEST)
            comment.restore()
